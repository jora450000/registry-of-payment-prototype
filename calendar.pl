...
#---------------------------------------------------------------------------------------------------------
	...
	@{$this->{weekdays}} = ("ÑÁ", "ÂÑ", "Ïí", "Âò", "Ñð", "×ò", "Ïò");

	# months: must start with January
	@{$this->{months}} = ("ßíâàðü", "Ôåâðàëü", "Ìàðò", "Àïðåëü", "Ìàé", "Èþíü", "Èþëü", "Àâãóñò", "Ñåíòÿáðü", "Îêòÿáðü", "Íîÿáðü", "Äåêàáðü");
...
#TestLine='ÀàÁáÂâÃãÄäÅå¨¸ÆæÇçÈèÉéÊêËëÌìÍíÎîÏïÐðÑñÒòÓóÔôÕõÖö×÷ØøÙùÚúÛûÜüÝýÞþßÿ';
...
#		 print "$my_year.$my_month.$my_day";
	...
	for ($i=1;$i< 32;++$i){
		...
		if ( -e $file){
  		 $myCal->viewEvent($i, $i, "#D0FFD0", "Ðååñòð", "./index.pl");#?date=".$my_year."-".$my_month."-" . $i ); 
	 }
	}
	...
	if ($my_day < 5){	
	  ...
	  for ($i=1;$i< 32;++$i){
		...
		$file = "c:/xampp/htdocs/".$my_year."-".(sprintf "%02d",$my_month)."-" . (sprintf "%02d",$i) .".htm";
		if ( -e $file){
	 	 $myCal2->viewEvent($i, $i, "#D0FFD0", "Ðååñòð", "./index.pl");#?date=".$my_year."-".$my_month."-" . $i ) ;
	   }
...
function clearDirty(){
	...
	<strong> Ðååñòð çà $mydate </strong>  
	...
	<input type= "SUBMIT"  value = "ïîäïèñàòü" onclick="clearDirty()">
	</form>
...
<script type="text/javascript">
	...
	document.body.onbeforeunload=function(){
		if(dirty) return "Ðåçîëþöèÿ íå îòïðàâëåíà, âûéòè?";
		else return null;
	};
</script>
...
#		$msg_body=MIME::Base64::Perl::encode_base64("Ðåçîëþöèÿ íà ðååñòð çà" . $mydate . "\n" . $data); 
#		$msg_body =Encode::from_to("Ðåçîëþöèÿ íà ðååñòð çà" . $mydate . "\n" . $data, "windows1251", "utf-8");
		$msg_body ="Ðåçîëþöèÿ íà ðååñòð çà" . $mydate . "\n" . $data;
		$msg_subj ="Ðåçîëþöèÿ íà ðååñòð çà" . $mydate;
